...
use application_database::entity::account::user::User;
...
pub async fn detail(id: i64) -> Result<User> {
    ...
    repository::account::user::fetch(id).await
}

pub async fn update_avatar(access_token: AccessToken, avatar: &str) -> Result<User> {
    repository::account::user::update_avatar(access_token.user_id, avatar).await
}

pub async fn update_nickname(access_token: AccessToken, nickname: &str) -> Result<User> {
    repository::account::user::update_nickname(access_token.user_id, nickname).await
}

pub async fn update_slogan(access_token: AccessToken, slogan: &str) -> Result<User> {
    repository::account::user::update_slogan(access_token.user_id, slogan).await
}

// todo: 对相同手机号进行账号数据合并
pub async fn update_phone(access_token: AccessToken, phone: &str) -> Result<User> {
    repository::account::user::update_phone(access_token.user_id, phone).await
}
